...
def read_file(filepath, race_id):
  ...
  partial_times = [header for header in raw_headers if "T.Int" in header or "Pos.Int" in header]
  headers = [
    "dorsal",
    "name",
    "generalPosition",
...
def format_state (state, got_intermedium):
  ...
  is_valid = re.findall(r'(finalizado|abandonado|abandono|no\s*empezado|running|descalificado|no\s*?terminado)', state.lower())
  if bool(is_valid):
    state = is_valid.pop()
    if not re.search(r'(no\s*empezado|no\s*finalizado|no\s*abandono|no\s*abandonado)', state) or got_intermedium:
