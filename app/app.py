...
from app.parser import read_file, format_state
...
def update(file, race_id):
  try:
    ...
    if not len(data):
      return True
    res = req.post('https://kilometrezero.org/rs/data/create_results.php', data=json.dumps(data)) # PRO
    # res = req.post('http://localhost/rs/data/create_results.php', data=json.dumps(data)) # DEV
    # res = req.post('https://kilometrezero.org/working/rs/data/create_results.php', data=json.dumps(data)) # PRE
    return True
...
class Application:
  ...
  def get_races(self):
    ...
    # r = req.get("http://localhost/rs/data/races.php?all=true") # DEV
    # r = req.get("https://kilometrezero.org/working/rs/data/races.php?all=true") # PRE
    return reversed(json.loads(r.text), key=lambda race: dt.fromtimestamp(int(int(race["date"])/1000)), reverse=True)
  ...
  def on_run(self):
    if self.running:
      ...
      self.running = True
      self.sync = Process(target=run_sync, args=(self.file, self.race, ))
      self.sync.start()
